...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            var result = Emps.Where(e => e.Job == "Backend programmer");
            return result;
        }
        ...
        {
            var result = Emps.Where(e => e.Job == "Frontend programmer" && e.Salary > 1000)
        ...
        {
            var result = Emps.Where(e => e.Salary == Emps.Max(sal => sal.Salary))
                             .ToList();
            return result;
        }
        ...
        public static IEnumerable<object> Task7()
        {
            IEnumerable<object> result = Emps.GroupBy(e => e.Job)
                                             .Select(e => new { Praca = e.Key, Count = e.Count()})
                                             .ToList();
            return result;
        }
        ...
        {
            bool result = Emps.isThereBackendProgrammer();
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            ...
            IEnumerable<Dept> result = Depts.GetDepts(Emps);
            return result;
        }
    }
    ...
    public static class CustomExtensionMethods
    {
        ...
        public static bool isThereBackendProgrammer(this IEnumerable<Emp> emps)
        {
            foreach (Emp e in emps)
            {
                if (e.Job.Equals("Backend programmer"))
                {
                    return true;
                }
            }
        ...
        public static IEnumerable<Dept> GetDepts(this IEnumerable<Dept> depts, IEnumerable<Emp> emps)
        {
            IDictionary<int, int> deptartments = new Dictionary<int, int>() 
            {
                {1, 0},
                {2, 0},
                {3, 0}
            ...
            foreach (Emp e in emps)
            {
                ...
                if (deptartments.ContainsKey((int)e.Deptno))
                {
                    deptartments.TryGetValue((int)e.Deptno, out count);
                    deptartments[(int)e.Deptno] = count + 1;
                }
            }
            foreach (KeyValuePair<int, int> entry in deptartments)
            {
