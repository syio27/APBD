...
namespace MovieApp.Server.Data
{
    public class ApplicationDbContext : ApiAuthorizationDbContext<ApplicationUser>
    {
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            builder.Entity<Movie>(opt =>
            {
                opt.Property(p => p.Title).IsRequired().HasMaxLength(300);
                opt.Property(p => p.Summary).IsRequired().HasMaxLength(300);
                opt.Property(p => p.InTheaters).IsRequired();
                opt.Property(p => p.Trailer).IsRequired().HasMaxLength(300);
                opt.Property(p => p.ReleaseDate).IsRequired().HasColumnType("datetime");
                opt.Property(p => p.Poster).IsRequired().HasMaxLength(300);
            });
